...
const sendPhoto = (chatId, photo, caption='') => {
  ...
  console.log(JSON.stringify(chatOptions, null, '  '));
  return request(chatOptions)
  .catch(function (err) {
    console.log("Main:", err.message);
  });
...
const build = (messageObj) => {
  ...
  if (event == "TRADE") {
    ...
    const zsymbol = currency + '-' + base;
    if (!trade_datas.symbol.hasOwnProperty(zsymbol)) {
      trade_datas.symbol[zsymbol] = {
        price: [],
        sell: [],
        buy: []
      }
    }
    trade_datas.symbol[zsymbol].price.push([today, price])
    trade_datas.symbol[zsymbol][trade].push({
      x: today,
      y: price,
      z: zvalue/1000,
    ...
    renderTradePhoto(title, trade_datas.symbol, zsymbol)
    .then(res => {
      sendPhoto(CHAT_ID, res)
    })
    .then((res) => {
      if(res && res.ok) {
    ...
    const foundInWhaleList = (p, q, whale) => {
      const zvalue = (+p)*(+q)*book.usd_price;
      return whale.filter(w => w.x === +p && w.z === zvalue/1000).length > 0
    }
    ...
    if (event == "VOLUME") {
      title = (typeof messageObj.type == "object") ? 
                `Order of ${(+messageObj.type[1]).toFixed(2)} ${currency} placed at ${(+messageObj.type[0]).toFixed(2)} ${base}` :
                `Total ${messageObj.side} volume = ${quantity} ${currency}, which is around ${messageObj.size} times bigger than counterpart`;

      // Only add to whale if current order is big
      const zvalue = (+messageObj.type[0])*(+messageObj.type[1])*book.usd_price;
      if (typeof messageObj.type == "object" && zvalue >= book.min_worth) {
    ...
    console.log(`DUMMY, no image for ${event} and type ${messageObj.type} message: asks ${range_text(asks)}, bids ${range_text(bids)}`);
  }

  // Continue sending text message
  return;
