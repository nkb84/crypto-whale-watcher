...
const build = (messageObj) => {
  ...
  if (event == "TRADE") {
    ...
    if (event == "VOLUME") {
      title = `order of ${messageObj.type[1]} ${currency} placed at ${messageObj.type[0]} ${base}`;
                `Total ${messageObj.side} volume = ${quantity} ${currency}, which is around ${messageObj.side} times bigger than counterpart`;
      if (typeof messageObj.type == "object") {
        if (messageObj.type[0] >= bids[0][0]) {
    ...
    console.log(`DUMMY, no send ${event} and type ${messageObj.type} message: asks ${asks[0][0]} ~ ${asks[asks.length - 1][0]}, bids ${bids[0][0]} ~ ${bids[bids.length - 1][0]}`);
  }
