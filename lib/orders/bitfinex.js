...
const bitfinex = (order) => {
  ...
  if(order.chanId) {
    ...
    if(order[1] != undefined && typeof order[1][0] == "number" && order[1].length == 3) {
      ...
      let usdExp = /^USD(T)?$/;
      const usdValue = (usdExp.test(base)?1:usd_values[base]);
      ...
      const buuk = {
        asks: [],
        bids: [],
        min_worth: min_worth[currency],
        usd_price: usdValue
      };
      book[channel_id].forEach((item, idx) => {
        if (item[2] < 0) {
          // Sell ==> bids
          buuk.bids.push([item[0], -1*item[2]])
        } else {
          // Buy ==> asks
          buuk.asks.push([item[0], item[2]])
        }
      })
