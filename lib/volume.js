...
const refresh_volumes = () => {
  ...
  getPairs("coinbase").forEach((symbol) => {
    tickerOptions.uri = `https://api.pro.coinbase.com/products/${symbol}/ticker`;  
    request(tickerOptions)
    .then(function (tickerResponse) {
      exchange_volumes.coinbase[symbol] = parseFloat(tickerResponse.volume);
    }).catch(function (err) {
